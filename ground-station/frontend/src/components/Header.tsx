        ...
        <div className="flex items-center gap-6 px-2 lg:px-4 text-xs">
          ...
          <div className="shrink-0">
            ...
            <div className="font-mono font-semibold text-slate-800">
              {Math.abs(uav.lat).toFixed(4)}° {uav.lat >= 0 ? 'N' : 'S'}, {Math.abs(uav.lng).toFixed(4)}° {uav.lng >= 0 ? 'E' : 'W'}
            </div>
          </div>
          <div className="w-px h-6 bg-slate-200 shrink-0" />
          <div className="shrink-0">
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-slate-500 truncate">
                            {Math.abs(d.lat).toFixed(4)}° {d.lat >= 0 ? 'N' : 'S'}, {Math.abs(d.lng).toFixed(4)}° {d.lng >= 0 ? 'E' : 'W'}
                          </div>
              ...
              <div className="absolute top-full right-0 mt-3 w-48 bg-white rounded-lg shadow-lg border border-slate-200 py-1 z-50">
                ...
                <button
                  onClick={() => {
                    ...
                    if (confirm('Log out of AERIS ground station?')) {
                      if (state.mission.status === 'active') state.endMission();
