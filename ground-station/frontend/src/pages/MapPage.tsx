...
export function MapPage() {
  ...
  useEffect(() => {
    ...
    const uavIcon = L.divIcon({
      className: 'custom-leaflet-marker bg-transparent border-none',
      html: `<div class="w-6 h-6 bg-white rounded-full border-2 border-aeris flex items-center justify-center text-aeris shadow-md transition-transform duration-300" style="transform: rotate(${uav.heading}deg)"><i class="ph-fill ph-navigation-arrow text-sm"></i></div>`,
      iconSize: [24, 24],
      iconAnchor: [12, 12],
    });
    ...
    <div className="h-full flex flex-col">
      <div className="panel-card min-h-full flex flex-col">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 border-b border-slate-100 pb-4">
          <div>
            <h2 className="text-xl font-bold text-slate-800 uppercase">Map</h2>
            <p className="text-xs text-slate-500 mt-1">2D Map view with real-time UAV position.</p>
          </div>
